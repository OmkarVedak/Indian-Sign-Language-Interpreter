...
def hog_single(img):
	...
	samples_inf.append(hist)
	return np.float32(samples_inf)
def trainSVM(num):
    imgs=[]
    ...
    labels = np.array(np.repeat(np.arange(1,num+1), 400)) # label for each corresponding image saved above
    print("hey")
    print(labels)
    # labels = np.vectorize(labels)
    # print(labels)
    samples=np.array(preprocess_hog(imgs))            # images sent for pre processeing using hog which returns features for the images 
